...
void CTank::DrawBackground()
{
    ...
    painter->drawText(m_BK.x() + m_wBK + 10,80,QString("得分为 %1").arg(m_Score));
}
...
void CTank::DrawTriangle()
{
	...
	for (i = 0; i < m_nTri; i++)
	{
		...
		h = m_Tri[i].h;
		w = m_Tri[i].w;
        painter->drawLine(x,y,x - w / 2,y + h);
        painter->drawLine(x - w / 2,y + h,x + w / 2, y + h);
        painter->drawLine(x + w / 2, y + h,x, y);
	}
}
